...
Objetivo: Verificar creación de slice con configuración mínima válida y topología lineal de 3 VMs
"""
...
class TestCase211:
    def __init__(self):
        ...
        self.test_results = {
            "case_number": "2.1.1",
            "objective": "Verificar creación de slice con configuración mínima válida y topología lineal de 3 VMs",
            "start_time": None,
            "end_time": None,
            "status": "PENDING",
            "steps": [],
    ...
    def check_prerequisites(self) -> bool:
        ...
        # Prerrequisito 3: Al menos un servidor disponible (ahora requerimos más para 3 VMs)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if len(servers) >= 3: # Cambiado para verificar al menos 3 servidores
                    ...
                    self.log_step(0, "Prerrequisito: Servidores disponibles", "FAIL", 
                                  {"error": f"Se requieren al menos 3 servidores, pero solo hay {len(servers)} disponibles"})
                    prerequisites_ok = False
            else:
                self.log_step(0, "Prerrequisito: Servidores disponibles", "FAIL", 
                              {"error": response.text})
    ...
    def execute_test_procedure(self) -> bool:
        """Ejecutar procedimiento del test"""
        print("\n🧪 EJECUTANDO PROCEDIMIENTO DE PRUEBA")
        print("-" * 40)
        
        # Paso 1: POST /slices con payload específico para 3 VMs en lineal
        slice_payload = {
            "name": "test-linear-slice-3vms",
            ...
            "networks": [
                {
                    "name": "net1",
                    "cidr": "192.168.1.0/24",
                    "connected_nodes": ["vm1", "vm2"] # Conecta vm1 y vm2
                },
                {
                    "name": "net2",
                    "cidr": "192.168.2.0/24",
                    "connected_nodes": ["vm2", "vm3"] # Conecta vm2 y vm3
                }
            ]
        }
        
        print("1. Enviando POST /slices con payload de 3 VMs en topología lineal...")
        try:
            ...
            try:
                ...
                # Paso 4: Verificar que se calcularon recursos totales correctamente (ajustar para 3 VMs)
                # Cada VM "small" tiene 2 vCPUs, 2048 MB RAM, 10 GB Disk
                # Para 3 VMs: 3 * 2 vCPUs = 6, 3 * 2048 MB RAM = 6144, 3 * 10 GB Disk = 30
                expected_resources = {
                    "total_vcpus": 6,
                    "total_ram": 6144,
                    "total_disk": 30
                ...
                # Verificar estado del slice
                slice_status = response_data.get('status')
                if slice_status == 'draft':
                    self.log_step(5, "Verificar estado 'draft'", "PASS", 
                                  {"expected": "draft", "actual": slice_status})
                else:
                    self.log_step(5, "Verificar estado 'draft'", "FAIL", 
                                  {"expected": "draft", "actual": slice_status})
                    return False
    ...
    def verify_database_state(self):
        """Verificar estado en base de datos (opcional)"""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_step(6, "Verificar slice en BD", "PASS", 
                              {"slice_id": self.slice_id, "data": slice_data})
                self.add_evidence("database_verification", slice_data)
                
                print(f"✅ Slice {self.slice_id} encontrado en BD:")
                print(f"   Nombre: {slice_data.get('name')}")
                print(f"   Estado: {slice_data.get('status')}")
                print(f"   Recursos: {slice_data.get('resources')}")
                # Aquí podrías añadir una verificación más exhaustiva de la topología
                # verificando que los nodos y redes coincidan con lo esperado para 3 VMs en lineal.
                print(f"   Nodos: {slice_data.get('nodes')}")
                print(f"   Redes: {slice_data.get('networks')}")

            else:
                self.log_step(6, "Verificar slice en BD", "FAIL", 
                              {"error": response.text})
        except Exception as e:
            self.log_step(6, "Verificar slice en BD", "FAIL", 
                          {"error": str(e)})
    ...
    def generate_test_report(self):
        ...
        if self.slice_id:
            print(f"\n🆔 SLICE CREADO: {self.slice_id}")
            print(f"   Para eliminar: curl -X DELETE {API_BASE}/slices/{self.slice_id}")
            print(f"   Con token: curl -X DELETE {API_BASE}/slices/{self.slice_id} -H 'Authorization: Bearer {self.token}'")
    ...
    def run_test(self):
        ...
        print("Creación de slice básico con topología lineal de 3 VMs")
        print("=" * 60)
        ...
        try:
            ...
            if self.execute_test_procedure():
                ...
                print("✅ Slice creado exitosamente con configuración mínima válida y topología lineal de 3 VMs")
                print("✅ Recursos calculados correctamente para 3 VMs")
                print("✅ Estado 'draft' asignado correctamente")
